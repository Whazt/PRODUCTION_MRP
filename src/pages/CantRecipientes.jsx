        ...
        {cantRecipientes !== null && (
          <div className="mt-4">
            <p className="text-gray-900 bg-green-300 p-3 rounded-md w-full">
              La cantidad de Recipientes es de: <strong>{cantRecipientes.toFixed(2)}</strong> {cantRecipientes % 1 !== 0 && `≈ ${Math.round(cantRecipientes - Math.floor(cantRecipientes) >= 0.5 ? Math.ceil(cantRecipientes) : Math.floor(cantRecipientes))}`}
            </p>
            <p className="text-gray-900 bg-green-300 p-3 rounded-md w-full">
              El inventario máximo es de: <strong>{nivelMax.toFixed(2)}</strong> {nivelMax % 1 !== 0 && `≈ ${Math.round(nivelMax - Math.floor(nivelMax) >= 0.5 ? Math.ceil(nivelMax) : Math.floor(nivelMax))}`}
            </p>
          </div>
        )}
      </div>
