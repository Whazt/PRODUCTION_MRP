...
function RotacionInventario() {
    ...
    const [errors, setErrors] = useState({
      ...
      const calculateRotation = () => {
        ...
        if (mode.m === '1') {
            const Invprom= (QorT/2 )+SS;
            const Rotacion = D/Invprom;
            setRotation(Rotacion);
            setInvprom(Invprom);
           
        } else if (mode.m === '2') {
            
            const Invprom=((D*QorT)/2)+SS;

            if(periodo === '1'){
                const Rotacion = (D*365)/Invprom;
                setRotation(Rotacion);
                setInvprom(Invprom);
            }else if(periodo === '2'){
                const Rotacion = (D*52)/Invprom;
                setRotation(Rotacion);
                setInvprom(Invprom);
            }else if(periodo === '3'){
                const Rotacion = (D*12)/Invprom;
                setRotation(Rotacion);
                setInvprom(Invprom);
            }
    ...
    return(
        ...
        <div className="max-w-md mx-auto mt-20 p-6 bg-gray-100 rounded-lg shadow-md">
            ...
            {rotation!== null && (
                ...
                <p className="text-gray-900 bg-green-300 p-3 rounded-md w-full">
                    La rotacion de inventario es de: <strong>{rotation.toFixed(2)}</strong>
                </p>
                </div>
            )}
        </div>
